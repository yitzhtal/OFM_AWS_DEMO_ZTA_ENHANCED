...
function TableListFiles(props) {
    ...
    const load = async () => {
        Storage.list('', {level: props.level, pageSize: 20, region: "us-east-1", bucket: "amplify-ofmawsdemoenhanced-dev-5108d-deployment"})
            .then(
                result => {
    ...
    function downloadFile(filename) {
        Storage.get(filename, {
            level: props.level, region: "us-east-1", bucket: "amplify-ofmawsdemoenhanced-dev-5108d-deployment"
        }).then(
            (result) => {
                openInNewTab(result);
            }
    ...
    function deleteFile(filename) {
        Storage.remove(filename, { level: props.level , region: "us-east-1", bucket: "amplify-ofmawsdemoenhanced-dev-5108d-deployment"}).then(
            (ok) => {
                load().then(
                    // alert("File deleted.")
                );
